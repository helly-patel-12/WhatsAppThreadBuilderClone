// src/components/modals/PhoneBrandModal.jsx
import { PhoneBrands } from '../../data/shopData';
...
export default function PhoneBrandModal({ selected, onChange, onContinue, onBack, onClose }) {
  return (
    <div className="popup-overlay">
      <div className="popup-box">
        ...
        <div className="popup-content d-flex">
          {PhoneBrands.map((b, i) => (
            <div key={i}>
              <input type="radio" name="brand" id={`brand-${i}`} value={b}
                checked={selected === b} onChange={() => onChange(b)} />
              <label className="popup-label" htmlFor={`brand-${i}`}>{b}</label>
            </div>
          ))}
        </div>
