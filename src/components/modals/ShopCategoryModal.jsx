// src/components/modals/ShopCategoryModal.jsx
...
export default function ShopCategoryModal({ selected, onChange, onContinue, onBack, onClose }) {
  return (
      ...
      <div className="popup-box">
        ...
        <div className="popup-content d-flex">
          {ShopCategories.map((cat, i) => (
            <div key={i}>
              <input type="radio" name="category" id={`category-${i}`} value={cat}
                checked={selected === cat} onChange={() => onChange(cat)} />
              <label className="popup-label" htmlFor={`category-${i}`}>{cat}</label>
            </div>
          ))}
        </div>
